...
```go
// hecc-blot-core/contract/sse/sse.go
type ISse interface {
...
// hecc-blot-core/contract/sse/sse_handler.go
...
SSE 复用 `hecc-blot-core/contract/api` 中的 `IMiddleware` 接口，无需单独定义中间件接口。
...
SSE 处理器接口定义在 `hecc-blot-core/contract/sse/sse_handler.go`：
