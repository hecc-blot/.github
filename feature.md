...
| # | 位置 | 问题 | 建议 |
|---|------|------|------|
| 1 | `framework/service/ioc` | `Container.values` 无锁，运行时并发 `Set` 有数据竞争 | ✅ 已约定：`framework/README.md`「IOC 容器」并发约定 + `ioc_svc.go` 注释明确 Set 仅初始化阶段调用 |
...
| 7 | 各模块 README | 文档全部中文，英文用户仅有 guide 的 README_EN.md | 后续翻译各模块 README |
